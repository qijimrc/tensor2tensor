...
class Transformer(t2t_model.T2TModel):
  ...
  def encode(self, inputs, target_space, hparams, features=None, losses=None):
    ...
    """
    inputs = common_layers.flatten4d3d(inputs)#[None,seq_len,hidden_dim]
  ...
  def body(self, features):
    ...
    if self.has_input:
      ...
      # 得到encoder输出, [None, None, 512]
      # 以及用作mask掉padding的bias
      encoder_output, encoder_decoder_attention_bias = (None, None)

    targets = features["targets"]
    targets_shape = common_layers.shape_list(targets)
    targets = common_layers.flatten4d3d(targets)#[None,None,512]

    ipdb.set_trace()
    # 对经过embedding之后的targets数据做预处理
    # 1. 得到bias,是一个下三角矩阵,上三角是负无穷,用于mask, [seq_len, seq_len]
    # 2. 对targets进行的第二个维度(seq_len)的最上面padding一行0
    # 3. 对targets加入时间(位置)信号
...
def transformer_prepare_encoder(inputs, target_space, hparams, features=None):
  ...
  # target_space到底是什么？为什么加入到encoder inputs中
  # target space应该是一个32维的one-hot特征，表征目的端某些位置标签？
  # 对每一个单词都一样, 所以加入到每一个单词的embedding向量中
  ...
  # 得到加入了target space特征之后的embedding向量 [None, None, 512]
...
def transformer_prepare_decoder(targets, hparams, features=None):
  ...
  if hparams.prepend_mode == "prepend_inputs_full_attention":
    ...
    # 返回一个下三角矩阵(上三角都是负无穷), [seq_len, seq_len]
    decoder_self_attention_bias = (
  ...
  with tf.variable_scope(name):
    ...
    for layer in range(hparams.num_decoder_layers or
      ...
      with tf.variable_scope(layer_name):
        with tf.variable_scope("self_attention"):
          # decoder每层的 multihead self-attention
          y = common_attention.multihead_attention(
              common_layers.layer_preprocess(x, hparams),
              None,
              decoder_self_attention_bias,
        ...
        # Decoder 每层对Encoder的attention

        # 预处理过的decoder inputs作为query, 之前得到的encoder
        # outputs作为key和value 做multihead attention
        if encoder_output is not None:
