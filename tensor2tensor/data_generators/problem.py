...
# 所有problem的基类， 根据数据路径以及参数，构建基本数据集
# 从本地数据路径中的tf records文件中读取数据，并转换成训练样本
...
# 构造数据集的基类problem->Text2TextProblem->TranslateProblem->TranslateEnZh
class Problem(object):
  ...
  def dataset(self,
    ...
    # 打断数据管道，查看每条样本数据
    #ipdb.set_trace()
    #test_iter = dataset.make_one_shot_iterator()
    #sess = tf.Session()
    #while(1):
    #    a_sample = sess.run(test_iter.get_next())
  ...
  def input_fn(self,
    ...
    def define_shapes(example):
      # 此处样本从[buket_size, sel_len]变成了 [?, ? , 1 ,1]
      # 单纯增加了维度
